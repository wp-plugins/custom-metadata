...
class custom_metadata_manager {
	...
	function _get_display_callback( $field ) {
		if( $field->display_callback && is_callable( $field->display_callback ) ) {
			return $field->display_callback;
		} elseif( in_array( $field->field_type, $this->_field_types ) ) {
			$callback = array( &$this, sprintf( '_display_%s_field', $field->field_type ) );
	...
	function _get_field_value( $field_slug, $field, $object_type, $object_id ) {
		...
		return get_metadata( $object_type, $object_id, $field_slug, true );
	}
	...
	function _display_metadata_field( $field_slug, $field, $object_type, $object_id ) {
		...
		$display_callback = $this->_get_display_callback( $field );
		
		if( $display_callback )
			return call_user_func( $display_callback, $field_slug, $field, $object_type, $object_id, $value );
	}
	
	function _display_text_field( $field_slug, $field, $object_type, $object_id, $value ) {
		?>
		<div class="custom-metadata-field">
			<label for="<?php echo $field_slug; ?>"><?php echo $field->label; ?></label>
			<input type="text" id="<?php echo $field_slug; ?>" name="<?php echo $field_slug; ?>" value="<?php echo esc_attr( $value ); ?>" />
	...
	function _display_radio_field( $field_slug, $field, $object_type, $object_id, $value ) {
		...
		<div class="custom-metadata-field">
			...
			<?php foreach( $field->values as $value_slug => $value_label ) : ?>
				<?php
				$checked = ( $value == $value_slug ) ? ' checked="checked"' : '';
				$value_id = sprintf( '%s_%s', $field_slug, $value_slug );
				?>
				<label for="<?php echo $value_id; ?>" class="selectit">
					<input type="radio" id="<?php echo $value_id; ?>" name="<?php echo $field_slug; ?>" value="<?php echo $value_slug ?>" <?php echo $checked; ?> />
	...
	function _display_select_field( $field_slug, $field, $object_type, $object_id, $value ) {
		...
		<div class="custom-metadata-field">
			...
			<select id="<?php echo $field_slug; ?>" name="<?php echo $field_slug; ?>">
				<?php foreach( $field->values as $value_slug => $value_label ) : ?>
					<?php
					$selected = ( $value == $value_slug ) ? ' selected="selected"' : '';
